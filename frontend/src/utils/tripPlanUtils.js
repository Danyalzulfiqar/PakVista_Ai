...
export const renderNotes = (notes) => {
  ...
  try {
      ...
      return (
        <div className="bg-cyan-900/40 border-l-4 border-cyan-400 p-3 rounded text-cyan-100 text-sm mb-2">
          ...
          <div className="space-y-2">
            {Object.entries(notes).map(([dayKey, dayData]) => (
              <div key={dayKey} className="border-l-2 border-cyan-500 pl-3">
                ...
                {dayData && dayData.activities && Array.isArray(dayData.activities) && (
                ...
                {dayData && dayData.budget && (
                  <div className="text-xs text-cyan-200 mt-1">
                    <span className="font-semibold">Budget:</span> {dayData.budget}
                  </div>
                )}
              </div>
            ))}
    ...
    if (Array.isArray(notes)) {
      return (
        <div className="bg-cyan-900/40 border-l-4 border-cyan-400 p-3 rounded text-cyan-100 text-sm mb-2">
          <span className="font-semibold text-cyan-300">Notes:</span> {notes.join(', ')}
        </div>
      );
    }
